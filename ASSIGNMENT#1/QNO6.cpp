...
int* GROW(int*& p_2, int& old_size, int& n_size) {
	cout << "ENTER NEW SIZE=";
	cin >> n_size;
	...
	if (n_size > old_size) {
		...
		cout << "ENTER NEW DATA OF "<< n_size-old_size<<" ELEMENTS" << endl;
		for (int i = old_size; i < n_size; i++)
		{
			cin >> newpntr[i];
		}
	}
...
int main() {
	...
	int* output = GROW(pntr, size, new_size);
	cout << "***********************OUTPUT***********************" << endl;
